...
for i in range(0,1):
    step = base*i
    url ="https://movie.douban.com/j/search_subjects?type=tv&tag=英剧&sort=recommend&page_limit=20&page_start={}".format(step)
    json_text = Package(url)
    Links = Urls(json_text=json_text)
    # titles = Titles(json_text=json_text) 
    # j=0

    print('At Page {}'.format(i+1))
...
print("Sortting Complete!\n{} Tv totally".format(len(TVs_sorted)))
...
for tv in TVs_sorted:
    i +=1
    print(u'Wrtting \t{}, \n{}Left'.format(tv['Name'],len(TVs_sorted)-i+1))
    ...
    if tv['Distribution'] is not None and tv['Distribution'] !='':
...
doc.save(u'英剧{}.docx'.format(datetime.date.today()))
